...
/**
 * 한 음절/특수문자에 대한 cho·jung·jong 또는 special img 마크업
 */
...
/**
 * @param {'minimal'|'full'|'fullIndented'} toolbar
 */
...
/**
 * 서버 /addWord JSON 배열 한 덩어리 → hangulWrapper HTML (test.html 의 addTest / editTest / syncWordLineBreak / generateTestSet 공통)
 *
 * @param {object} opts
 * @param {object[]} opts.jsonArray
 * @param {string} opts.fullWord
 * @param {number|string} opts.ordering
 * @param {'editor'|'compact'} opts.profileKey
 * @param {'minimal'|'full'|'fullIndented'} opts.toolbar
 * @param {string} [opts.rowWrapperStyle] — 예: syncWordLineBreak 에서 width 제거
 */
